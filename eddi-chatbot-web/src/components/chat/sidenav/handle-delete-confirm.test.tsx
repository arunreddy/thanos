...
describe("SideNav handleDeleteConfirm Function", () => {
  test("handles deletion of active chat", async () => {
    // Reset all mocks
    vi.clearAllMocks();
  ...
  test("handles deletion of non-active chat", async () => {
    ...
    // Find the first chat item and click its delete button
    const deleteButtons = await screen.findAllByLabelText(
      "Delete conversation"
    );
    await act(async () => {
      fireEvent.click(deleteButtons[0]); // Delete the first chat (id: "1")
    });
  ...
  test("handles null chatToDelete", async () => {
    // Reset all mocks
    vi.clearAllMocks();
  ...
  test("tests branch coverage for try-catch block", async () => {
    // Reset all mocks
    vi.clearAllMocks();
    ...
    // Create a custom component that directly tests the try-catch block
    const TestComponent = () => {
      const handleDeleteConfirm = async () => {
        const id = "test-id";
        if (!id) return;
